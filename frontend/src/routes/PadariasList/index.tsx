...
const PadariasList = ({ user }: Props) => {
	...
	const [isDoneValidatingInputCep, setIsDoneValidatingInputCep] =
	...
	return (
		<div id='padarias-list'>
			{isFetching ||
				(isValidatingInputCep && (
					<LinearProgress className='linear-progress' />
				))}

			<div className='header'>
				{user == undefined && CepInput()}
				{user == undefined &&
					!isFetching &&
					!submitedCep &&
					!isDoneFetching && <h2>Insira seu CEP</h2>}
				{isDoneFetching && !inputCepIsInvalid && padarias.length > 0 && (
			...
			<div className='grid'>
				{padarias?.map((padaria) => (
					<PadariaCard padaria={padaria} key={padaria.id} />
				))}
			</div>
